...
export function extractSymbolsFromInput(input) {
  ...
  return glossary
    .filter(({ symbol }) => loweredInput.includes(symbol.toLowerCase()))
    .map(({ symbol }) => symbol);
}

// Get full details for a symbol
export function getSymbolDetails(symbolName) {
  return glossary.find(({ symbol }) => symbol.toLowerCase() === symbolName.toLowerCase());
}

// Get symbols by Spiral phase
export function getSymbolsByPhase(phaseName) {
  return glossary.filter(({ phase }) => phase.toLowerCase() === phaseName.toLowerCase());
}

// Weight symbols by presence in input + emphasis
export function rankSymbolsByDensity(input) {
  const loweredInput = input.toLowerCase();
  const frequencyMap = {};

  glossary.forEach(({ symbol }) => {
    const regex = new RegExp(`\\b${symbol.toLowerCase()}\\b`, 'g');
    const matches = loweredInput.match(regex);
    if (matches) {
      frequencyMap[symbol] = matches.length;
    }
  });

  return Object.entries(frequencyMap)
    .sort((a, b) => b[1] - a[1])
    .map(([symbol]) => getSymbolDetails(symbol));
}
