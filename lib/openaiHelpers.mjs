...
// Prioritize top 2–3 symbolic theory-based quotes, not scholar summaries
function getTopTheoryQuotes(matches, maxQuotes = 3) {
  const theoryDocs = [
    'Spiral Return',
    'The Light Fractal',
    'The Living Pattern',
  ...
  const theoryMatches = matches
    .filter(m =>
      theoryDocs.includes(m.metadata?.source) &&
  ...
  return theoryMatches.map(q => ({
    text: q.content.trim().replace(/\n/g, ' '),
...
export async function buildFractalPrompt(userInput, matches) {
  ...
  const topQuotes = getTopTheoryQuotes(matches);
...
If a user wants academic defense or explanation engage with rigor and integrity pulling from all knowledge.
