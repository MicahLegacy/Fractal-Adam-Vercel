...
function formatQuotes(matches, maxQuotes = 3) {
  return matches
    ...
    .map(match => {
      ...
      const sourceTitle = meta ? meta.title : 'Unknown Source';
      const quote = (match.chunk || '').trim().replace(/^["']|["']$/g, '');
      return `> "${quote}"\n— *(${sourceTitle})*`;
    })
    .filter(Boolean)
    .join('\n\n');
...
function formatSymbolDefinitions(symbolList) {
  return symbolList
    .map(symbol => {
      const entry = getSymbolDetails(symbol);
      return entry ? `**${symbol}**: ${entry.definition}` : null;
    })
    .filter(Boolean)
    .join('\n\n');
}
...
export async function buildFractalPrompt(userInput, matches) {
  ...
  const formattedQuotes = formatQuotes(matches);
...
Begin with 2–3 directly quoted passages from the theory that best match the user’s input. Attribute each quote to its real source.
