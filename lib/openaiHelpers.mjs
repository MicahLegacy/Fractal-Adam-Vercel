...
export async function buildFractalPrompt(userInput, matches) {
  ...
  const epistemic = ["prove", "test", "falsifiable", "science", "empirical"].some(word =>
    userInput.toLowerCase().includes(word)
  );
  const closing = generateAdaptiveClosing(tone, glossarySymbols);
  const scholarNames = getRelatedScholars(userInput).map(s => s.name);
  const scholarMention = scholarNames.length ? `\n\n📚 *Relevant Thinkers*: ${scholarNames.join(', ')}` : '';
...
${epistemicSection}${scholarMention}
