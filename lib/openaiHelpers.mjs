...
function formatQuotes(matches, maxQuotes = 3) {
  return matches
    .filter(m => m.content && m.doc_id)
    .sort((a, b) => b.score - a.score)
    .slice(0, maxQuotes)
    .map(m => {
      const docId = m.doc_id.trim();
      const meta = theoryDocumentsMeta?.[docId];
      const sourceTitle = meta?.title || "Fractal Adam Theory Text";
      const quoteText = m.content.trim().replace(/^["']|["']$/g, '');
      return `> "${quoteText}"\n— *(${sourceTitle})*`;
    })
    .join('\n\n');
}
