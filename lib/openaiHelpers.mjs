...
// 🆕 Multi-Quote Injection (Step 8)
function getTopQuotes(matches, maxQuotes = 3) {
  return matches
    .filter(m => m.content.length > 100 && m.content.length < 500)
    .sort((a, b) => b.score - a.score)
    .slice(0, maxQuotes)
    .map((m, i) => {
      const cleanQuote = m.content.trim().replace(/\n/g, ' ');
...
Cite scholars such as Jung, Bateson, Prigogine, Varela, Porges, Mandelbrot, and any other relevent scholars when defending theory claims using science.
Cite other relevent domain thinkers as needed with precision and integrity.
