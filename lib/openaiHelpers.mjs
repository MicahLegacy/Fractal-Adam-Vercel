...
function formatQuotes(matches, maxQuotes = 3) {
  const quotes = matches
    ...
    .map((match) => {
      ...
      const cleanText = match.chunk?.trim()?.replace(/^["']|["']$/g, "") || "";
      return `> ${cleanText}\n— *(${source})*`;
    });
  return quotes.join('\n\n');
}

function formatSymbolDefinitions(symbolList, symbolDefs) {
  return symbolList
    .map(symbol => {
...
export async function buildFractalPrompt(userInput, matches) {
  ...
  const scholarNames = scholars.map(s => s.name).join(', ') || "none";
  const formattedQuotes = formatQuotes(matches, 3);
  const inlineSymbolDefs = formatSymbolDefinitions(symbolList, symbolDefs);
...
Begin with 2–3 directly quoted passages from the theory that best match the user’s input. Attribute each quote to its source using the document title.
