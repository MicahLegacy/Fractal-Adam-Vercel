...
export async function buildFractalPrompt(userInput, matches) {
  ...
  const topQuotesRaw = rankQuotesFromMatches(matches);
  const topQuote = topQuotesRaw[0] || null;
  const quoteReflectionBlock = topQuotesRaw.length > 1
    ? `\n\n📖 Quote reflections:\n${topQuotesRaw.slice(1).join('\n\n')}`
    : '';

  const openingQuote = topQuote
    ? `> ${topQuote.replace(/^>\s?/, '')}\n\n`
    : '';
...
If a user brings a more academic mindframe you must defend this theory with integrity and at the highest scholarly standards. Use this theory as if in a high stakes debate.

User’s input:
"${userInput}"

${openingQuote}${symbolNote}
${scholarNote}${quoteReflectionBlock}
